...
tree_mode()
{
	...
	# Display each window with window_id
	# tmux lsw -t"$s" -F'#{window_id}' | while read -r w; do
	#   W=$(tmux lsw -t"$s" -F'#{window_id}#{T:tree_mode_format}' | grep ^"$w")
	#   echo "  ﬌ ${W##$w}"
	tmux lsw -t"$s" -F'#{window_id}' | while read -r w; do
	W=$(tmux lsw -t"$s" -F'#{window_id}#{window_name}: #{T:tree_mode_format}' | grep ^"$w")
	if [[ -n "$highlight" ]] && [[ "$highlight" == "$session_name" ]]; then
		echo -e "  \033[1;34m﬌ ${W##$w}\033[0m"
	else
		echo -e "  \033[1m﬌ ${W##$w}\033[0m"
	fi
	# Display each pane with pane_id
	tmux lsp -t"$s:$w" -F'#{pane_id}' | while read -r p; do
	P=$(tmux lsp -t"$s:$w" -F'#{pane_id}#{T:tree_mode_format}' | grep ^"$p")
	if [[ -n "$highlight" ]] && [[ "$highlight" == "$session_name" ]]; then
		echo -e "    \033[1;34m﬌ ${P##$p}\033[0m"
	else
		echo -e "    \033[1m﬌ ${P##$p}\033[0m"
	fi
done
done
done
